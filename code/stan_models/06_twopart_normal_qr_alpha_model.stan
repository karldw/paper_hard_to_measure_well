...
#include 06_twopart_normal_qr_alpha_param.stan
model {
  ...
  // Alpha is quite small
  cost_alpha ~ normal(0, 1); // folded normal prior b/c cost_alpha is bounded.
  ...
  if (!prior_only) {
    ...
    // Model alpha
    vector[N] log_e_log_p = log_emiss + log_price;
    vector[N] mu_alpha = log(-cost_alpha) +
        (cost_alpha - 1) * log_inv_logit(temp_obs_inter + Q * b_obs_qr);
    log_e_log_p ~ normal(mu_alpha, sigma_alpha);
  }
}
